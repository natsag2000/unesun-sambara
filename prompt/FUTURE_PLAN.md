...
- **P9 (WS-01..WS-06, WS-09)** `[x]` **XL** (sum of parts — see the plan doc's
  ...
  see the impl log for the full design. Then **WS-08 partial**: the
  popup never appeared on mobile devices *at all* (reported by the user
  testing on a real phone) because virtual-keyboard input goes through
  `#mobile-input`'s `input` event, which never triggered a refresh -
  fixed, along with mobile keydown popup-navigation and touch-tap
  support. Popup *positioning* on small/keyboard-covered mobile
  viewports remains unaddressed. `WS-07` (phrase-aware suggestions)
  remains fully deferred, exactly as scoped in the plan's §12.
  - Impl log: [`DOC/IMPL/WORD_SUGGESTIONS_IMPL.md`](../DOC/IMPL/WORD_SUGGESTIONS_IMPL.md) · Patches: `patch/word-suggestions.patch`, `patch/word-suggestions-arrow-nav-fix.patch`, `patch/word-suggestions-ws09.patch`, `patch/word-suggestions-mobile-trigger.patch`.
...
14. **WS-08: the popup didn't appear on mobile at all - full positioning rework, or fix the trigger gap first?** — **Resolved (fix the trigger gap; positioning deferred).** The user reported testing on a real mobile device and finding the popup never showed up, full stop - a much more fundamental problem than the "positioning on a small/keyboard-covered viewport" concern §11.4 originally anticipated. Root cause: virtual keyboards drive text entry through `#mobile-input`'s `input` event, which never called `refresh()` at all (a gap already flagged, but whose severity wasn't clear until tested on a device). Fixed that, plus mobile keydown popup-navigation (for a connected physical/Bluetooth keyboard) and touch-tap-to-accept on the popup's own canvas. Popup positioning on small/keyboard-covered viewports remains out of scope for this pass - `WS-08` is tracked as partially done (`[~]`), not closed. See `DOC/IMPL/WORD_SUGGESTIONS_IMPL.md`'s WS-08 section.
